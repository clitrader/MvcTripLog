...
namespace MvcTripLog.Controllers
{
    public class TripsController : Controller
    {
        ...
        // GET: Trips
        public ActionResult Index(string tripRegion, string searchString)
        {
            var RegionLst = new List<string>();

            var RegionQry = from d in db.Trips
                           orderby d.Region
                           select d.Region;

            RegionLst.AddRange(RegionQry.Distinct());
            ViewBag.tripRegion = new SelectList(RegionLst);

            var movies = from m in db.Trips
                         select m;
            ...
            if (!string.IsNullOrEmpty(tripRegion))
            {
                movies = movies.Where(x => x.Region == tripRegion);
            }
